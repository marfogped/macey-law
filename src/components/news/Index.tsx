  ...
  return (
    <section className="w-full h-max bg-[#0F0F1C] py-12">
      ...
      >
        ...
        <h3 className="section-text text-center text-neutral/80">
      ...
      <div className="section-container grid xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-5 py-8 gap-5">
        {NEWS_ARTICLES.map((article, articleIdx) => (
          ...
          >
            ...
            <div
              className={`absolute flex flex-col top-12 h-[21rem] w-full bg-neutral px-3 py-1.5 transition-transform card-shadow duration-500 ${
                openDetails[article.id] ? "translate-y-0" : "translate-y-full"
              }`}
            >
              <div className="flex flex-col flex-grow">
                <div className="mb-2">
                  <h4 className="text-pretty font-semibold text-lg font-raleway">
                    {article.title}
                  </h4>
                  <hr className="w-full bg-[#0F0F1C] h-[2px]" />
                </div>
                <div className="flex-grow">
                  <p className="text-balance xs:text-md sm:text-md md:text-sm font-lato font-light text-black/80">
                    {article.description}
                  </p>
                </div>
              </div>
              <div className="mt-auto">
                {article.href && article.href.length ? (
                  ...
                  >
                    See {article.title.toUpperCase()} Note{" "}
